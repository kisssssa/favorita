...
# Конфиг
# =========================
TRAIN_FILLED_PATH = "train_filled_active_period.parquet"
...
OUTPUT_DIR = Path("outputs_lightgbm_filled_active_period_raw_eval")
...
def load_and_prepare_features(force_rebuild: bool = False):
    ...
    print("Чтение заполненного train ...")
    df = pd.read_parquet(TRAIN_FILLED_PATH)

    if TRAIN_START_DATE is not None:
        df = df[df["date"] >= pd.Timestamp(TRAIN_START_DATE)].copy()
    ...
    # onpromotion в filled-датасете уже должен быть bool, но перестрахуемся
    ...
    print("Построение календарных lag/rolling признаков ...")
    ...
    print("Формирование valid_truth только по исходным raw-наблюдениям ...")
    ...
    if TRAIN_START_DATE is not None:
        raw_train = raw_train[raw_train["date"] >= pd.Timestamp(TRAIN_START_DATE)].copy()

    raw_train["unique_id"] = raw_train["store_nbr"].astype(str) + "_" + raw_train["item_nbr"].astype(str)
    raw_train["unit_sales_nonneg"] = raw_train["unit_sales"].clip(lower=0).astype("float32")

    uid_item_meta = (
        df[["unique_id", "item_nbr", "perishable"]]
        .drop_duplicates(subset=["unique_id"])
        .copy()
    )

    valid_truth = raw_train[raw_train["date"] >= valid_start][["unique_id", "date", "unit_sales_nonneg"]].copy()
    valid_truth = valid_truth.merge(uid_item_meta, on="unique_id", how="left")
...
def run_lightgbm(train_feat, valid_known, valid_truth, history_prevalid, meta):
    ...
    print("\n========== LightGBM on filled active-period data, raw eval ==========")
    study = optuna.create_study(
        study_name="favorita_lightgbm_filled_active_period_raw_eval",
        direction="minimize",
...
def main():
    ...
    print(f"TRAIN_FILLED_PATH = {TRAIN_FILLED_PATH}")
    print(f"RAW_TRAIN_PATH = {RAW_TRAIN_PATH}")
