...
OUTPUT_DIR = Path("outputs_lightgbm_weekday")
...
LGBM_MODEL_DIR = OUTPUT_DIR / "lightgbm_weekday_models"
LGBM_MODEL_DIR.mkdir(exist_ok=True, parents=True)
...
def load_and_prepare_features(force_rebuild: bool = False):
    ...
    valid_known_cols = ["unique_id", "date", "dayofweek"] + STATIC_CAT_COLS + KNOWN_NUM_COLS
...
def fit_lightgbm_single(fit_df: pd.DataFrame, eval_df: pd.DataFrame, params: dict):
...
def recursive_predict_valid_weekday(
    models_by_dow: Dict[int, lgb.LGBMRegressor],
    ...
    for current_date in valid_dates:
        ...
        for dow in range(7):
            sub = day_df[day_df["dayofweek"] == dow].copy()
            if len(sub) == 0:
                continue

            X_sub = sub[FEATURE_COLS].copy()
            for col in STATIC_CAT_COLS:
                X_sub[col] = X_sub[col].astype("category")

            model = models_by_dow[dow]
            pred_log = model.predict(X_sub, num_iteration=model.best_iteration_)
            pred = inverse_log_target(pred_log)

            sub_pred = sub[["unique_id", "date"]].copy()
            sub_pred["y_pred"] = pred
            day_preds.append(sub_pred)

        day_pred = pd.concat(day_preds, ignore_index=True)
        pred_parts.append(day_pred)

        merged = day_df[["unique_id"]].merge(day_pred, on="unique_id", how="left")
        merged["y_pred"] = merged["y_pred"].fillna(0.0)

        for uid, p in zip(merged["unique_id"].values, merged["y_pred"].values):
...
def make_lgbm_weekday_objective(train_feat, valid_known, valid_truth, history_prevalid, meta):
    ...
    def objective(trial: optuna.Trial):
        ...
        models_by_dow = {}
        for dow in range(7):
            fit_d = fit_df[fit_df["dayofweek"] == dow].copy()
            eval_d = eval_df[eval_df["dayofweek"] == dow].copy()
            models_by_dow[dow] = fit_lightgbm_single(fit_d, eval_d, params)

        _, metrics = recursive_predict_valid_weekday(
            models_by_dow=models_by_dow,
...
def run_lightgbm_weekday(train_feat, valid_known, valid_truth, history_prevalid, meta):
    ...
    study = optuna.create_study(
        study_name="favorita_lightgbm_weekday",
        direction="minimize",
    ...
    print(f"LightGBM weekday Optuna: completed={completed}, remaining={remaining}")

    if remaining > 0:
        objective = make_lgbm_weekday_objective(train_feat, valid_known, valid_truth, history_prevalid, meta)
    ...
    print("LightGBM weekday best params:", best_params)

    fit_df, eval_df = split_train_inner_eval(train_feat, meta["inner_eval_start"])

    models_by_dow = {}
    for dow in range(7):
        print(f"Обучение модели для dayofweek={dow}")
        fit_d = fit_df[fit_df["dayofweek"] == dow].copy()
        eval_d = eval_df[eval_df["dayofweek"] == dow].copy()
        model = fit_lightgbm_single(fit_d, eval_d, best_params)
        models_by_dow[dow] = model
        model.booster_.save_model(str(LGBM_MODEL_DIR / f"lightgbm_dow_{dow}.txt"))

    pred_df, metrics = recursive_predict_valid_weekday(
        models_by_dow=models_by_dow,
    ...
    print("LightGBM weekday metrics:", metrics)
...
def main():
    ...
    run_lightgbm_weekday(train_feat, valid_known, valid_truth, history_prevalid, meta)
