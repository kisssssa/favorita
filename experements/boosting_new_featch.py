...
OUTPUT_DIR = Path("outputs_lightgbm_plus_ts_features")
...
DYNAMIC_NUM_COLS = BASE_DYNAMIC_NUM_COLS + NEW_TS_FEATURES
...
def compute_days_since_last_nonzero(group: pd.DataFrame) -> pd.Series:
    values = group["unit_sales_nonneg"].to_numpy()
    ...
    for i, val in enumerate(values):
        if last_nonzero_idx == -1:
            ...
            result[i] = float(i - last_nonzero_idx)
...
def load_and_prepare_features(force_rebuild: bool = False):
    ...
    grp_sales = df.groupby("unique_id")["unit_sales_nonneg"]
    grp_promo = df.groupby("unique_id")["onpromotion"]
    ...
    for window in ROLLING_WINDOWS:
        df[f"rolling_mean_{window}"] = (
            grp_sales.transform(lambda s: s.shift(1).rolling(window, min_periods=1).mean())
            .astype("float32")
        )

    df["rolling_std_7"] = (
        grp_sales.transform(lambda s: s.shift(1).rolling(7, min_periods=1).std())
        .astype("float32")
    )
    ...
    df["rolling_min_7"] = (
        grp_sales.transform(lambda s: s.shift(1).rolling(7, min_periods=1).min())
        .astype("float32")
    )
    df["promo_sum_7"] = (
        grp_promo.transform(lambda s: s.shift(1).rolling(7, min_periods=1).sum())
        .astype("float32")
    )

    print("Построение days_since_last_nonzero_sale ...")
    df["days_since_last_nonzero_sale"] = (
        df.groupby("unique_id", group_keys=False)
          .apply(compute_days_since_last_nonzero)
          .astype("float32")
    )

    df["rolling_max_7"] = df["rolling_max_7"].fillna(0).astype("float32")
    df["rolling_min_7"] = df["rolling_min_7"].fillna(0).astype("float32")
    df["promo_sum_7"] = df["promo_sum_7"].fillna(0).astype("float32")
    df["days_since_last_nonzero_sale"] = df["days_since_last_nonzero_sale"].fillna(999).astype("float32")
...
def compute_dynamic_features_for_day(day_df: pd.DataFrame, history: Dict[str, list[float]]) -> pd.DataFrame:
    ...
    for uid in day_df["unique_id"].values:
        ...
        def get_roll_max(k: int):
            if len(hist) == 0:
                return 0.0
            arr = hist[-k:] if len(hist) >= k else hist
            return float(np.max(arr))

        def get_roll_min(k: int):
            if len(hist) == 0:
                return 0.0
            arr = hist[-k:] if len(hist) >= k else hist
            return float(np.min(arr))

        def get_days_since_last_nonzero():
            if len(hist) == 0:
                return 999.0
            for back, val in enumerate(reversed(hist), start=1):
                if val > 0:
                    return float(back)
            return 999.0
        ...
        rmax_7.append(get_roll_max(7))
        rmin_7.append(get_roll_min(7))
        days_since.append(get_days_since_last_nonzero())
...
def run_lightgbm(train_feat, valid_known, valid_truth, history_prevalid, meta):
    ...
    study = optuna.create_study(
        study_name="favorita_lightgbm_plus_ts_features",
        direction="minimize",
